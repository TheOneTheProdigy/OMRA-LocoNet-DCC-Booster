...
void loop() {
  ...
  if (BOOST1_AMPS >= OC_LIMIT && IS_POWER1_TRIPPED == false && IS_POWER1_INSTANT_PRE_TRIPPED == false) {
      POWER1_INSTANT_PRE_TIME = CURRENT_CYCLE_TIME;
      IS_POWER1_INSTANT_PRE_TRIPPED = true;
    }
    else if (IS_POWER1_INSTANT_PRE_TRIPPED && BOOST1_AMPS < OC_LIMIT && (CURRENT_CYCLE_TIME - POWER1_INSTANT_PRE_TIME) >= OC_BOOST1_INRUSH) {
      IS_POWER1_INSTANT_PRE_TRIPPED = false;
    }

  if (IS_POWER1_INSTANT_PRE_TRIPPED && IS_POWER1_TRIPPED == false && (CURRENT_CYCLE_TIME - POWER1_INSTANT_PRE_TIME) >= OC_BOOST1_INRUSH) {
  ...
  // Over current fault trip routine  
  if (BOOST2_AMPS >= OC_LIMIT && IS_POWER2_TRIPPED == false && IS_POWER2_INSTANT_PRE_TRIPPED == false) {
      POWER2_INSTANT_PRE_TIME = CURRENT_CYCLE_TIME;
      IS_POWER2_INSTANT_PRE_TRIPPED = true;
    }
    else if (IS_POWER2_INSTANT_PRE_TRIPPED && BOOST2_AMPS < OC_LIMIT && (CURRENT_CYCLE_TIME - POWER2_INSTANT_PRE_TIME) >= OC_BOOST2_INRUSH) {
      IS_POWER2_INSTANT_PRE_TRIPPED = false;
    }

  if (IS_POWER2_INSTANT_PRE_TRIPPED && IS_POWER2_TRIPPED == false && (CURRENT_CYCLE_TIME - POWER2_INSTANT_PRE_TIME) >= OC_BOOST2_INRUSH) {
  ...
  if ((IS_POWER1_TRIPPED == true) && ((CURRENT_CYCLE_TIME - BOOSTER1_SHUTDOWN_TIME) >= OC_TIMEOUT)) { // Retry if timer resets.
      turnPower1On();
  }

  // Booster 2 Repower Procedure
  if ((IS_POWER2_TRIPPED == true) && ((CURRENT_CYCLE_TIME - BOOSTER2_SHUTDOWN_TIME) >= OC_TIMEOUT)) { // Retry if timer resets.
      turnPower2On();
  }
  ...
  if (millis() - LAST_PRINT_DISPLAY_TIME >= PRINT_DISPLAY_DELAY_TIME) {
    ...
    if (IS_POWER1_INSTANT_PRE_TRIPPED == true) {     
      display.drawBitmap(32, 0, ALARM1ICON, 16, 16, WHITE);   
    }
    if (IS_POWER2_INSTANT_PRE_TRIPPED == true) {     
  ...
  if ((millis() - PWM_TIMER) > (PWM_TIMER_LIMIT)) {  
    if ((PWM_COUNT >= PWM_SIG_EDGES) && (IS_POWER1_TRIPPED == false)) {
      turnPower1On();
    }
    if ((PWM_COUNT >= PWM_SIG_EDGES) && (IS_POWER2_TRIPPED == false)) {
      turnPower2On();
    }
  ...
  // Debug Code
   if ((PRINT_DEBUG == true) && (CURRENT_CYCLE_TIME - LAST_DEBUG_REFRESH_TIME >= DEBUG_REFRESH_TIME)) {
